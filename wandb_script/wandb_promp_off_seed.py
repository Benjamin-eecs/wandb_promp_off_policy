...
def main(tune_config = None):
    ...
    with sweep_run:
        ...
        dump_path = meta_policy_search_path + '/data/pro-mp/test_%s_seed_%d' % (sweep_run.name, default_config['seed'])
        # configure logger
        ...
        for itr in range(default_config['n_itr']):
            wandb.log({'Step_1-AverageReturn':Step_1_AverageReturn[itr], 'test-Step_1-AverageReturn':test_Step_1_AverageReturn[itr]})
